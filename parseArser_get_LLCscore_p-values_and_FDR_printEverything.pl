...
# This script does not filter on fold change or p-value.

my $usage = "Usage:\n$0 <Arser LLC score file> <FDR> <mean> <stddev>\n";
...
my $LLC_outFile = 'LLC_stats_FDR'. $FDR . '_ARS.Old_mean' . $mean . '_stdDev' . $stdDev . '_noFilter.txt';

my($rhythmVals) = readLLCfile($LLCfile);
my($LLCstats,$ELCstats) = getPVals($rhythmVals,$mean,$stdDev); # I don't currently use ELCstats
my($sigInfo_LLCs) = BH_test($LLCstats,$FDR);
printLLCinfo($sigInfo_LLCs,$LLC_outFile);
